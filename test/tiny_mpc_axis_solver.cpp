...
TEST(tiny_mpc_axis_solver, solves_constant_reference) {
    ...
    auto result = solver.solve_center_kinematics(make_constant_reference(0.75));
    ASSERT_TRUE(result.has_value()) << result.error();
    EXPECT_NEAR(result->angle, 0.75, 1e-6);
}

TEST(tiny_mpc_axis_solver, solve_center_kinematics_requires_initialization) {
    auto solver = TinyMpcAxisSolver {};

    auto result = solver.solve_center_kinematics(make_constant_reference(0.25));
...
TEST(tiny_mpc_axis_solver, can_reinitialize_without_reset) {
    ...
    auto result = solver.solve_center_kinematics(make_constant_reference(0.5));
    ASSERT_TRUE(result.has_value()) << result.error();
    EXPECT_NEAR(result->angle, 0.5, 1e-6);
}
...
TEST(tiny_mpc_axis_solver, can_solve_multiple_references_after_initialization) {
    ...
    auto first = solver.solve_center_kinematics(make_constant_reference(0.25));
    ASSERT_TRUE(first.has_value()) << first.error();
    EXPECT_NEAR(first->angle, 0.25, 1e-6);

    auto second = solver.solve_center_kinematics(make_constant_reference(-0.5));
    ASSERT_TRUE(second.has_value()) << second.error();
    EXPECT_NEAR(second->angle, -0.5, 1e-6);
}
