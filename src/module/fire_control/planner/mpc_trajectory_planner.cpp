...
auto to_duration(double seconds) -> rmcs::Duration {
    return std::chrono::duration_cast<rmcs::Duration>(std::chrono::duration<double> { seconds });
}
...
struct MpcTrajectoryPlanner::Impl {
    ...
    auto initialize(YAML::Node const& yaml, AimPointChooser::Config const& config_in) noexcept
    ...
    auto plan(predictor::Snapshot const& snapshot, TimePoint center_time, double bullet_speed,
        ...
        const auto yaw = yaw_solver.solve_center(extract_axis_reference(reference->trajectory, 0));
...
private:
    static auto sample_at(predictor::Snapshot const& snapshot, TimePoint t, double bullet_speed,
        ...
        const auto target_d = std::hypot(armor_position.x(), armor_position.y());
    ...
    auto generate_reference(predictor::Snapshot const& snapshot, TimePoint center_time,
        ...
        for (int i = 0; i < static_cast<int>(samples.size()); ++i) {
            const auto offset = (i - (kMpcAxisHalfHorizon + 1)) * kMpcAxisDt;
            const auto t      = center_time + to_duration(offset);
            auto sample = sample_at(snapshot, t, bullet_speed, yaw_offset, pitch_offset, chooser);
            if (!sample.has_value()) return std::nullopt;
            samples[i] = *sample;
        }

        const auto yaw_origin   = samples[kMpcAxisHalfHorizon + 1].yaw;
        const auto target_yaw   = yaw_origin;
        const auto target_pitch = samples[kMpcAxisHalfHorizon + 1].pitch;

        auto trajectory = ReferenceTrajectory { };
        for (int i = 0; i < kMpcAxisHorizon; ++i) {
            const auto& previous = samples[i];
            const auto& current  = samples[i + 1];
            const auto& next     = samples[i + 2];

            const auto yaw_velocity =
                util::normalize_angle(next.yaw - previous.yaw) / (2.0 * kMpcAxisDt);
            const auto pitch_velocity = (next.pitch - previous.pitch) / (2.0 * kMpcAxisDt);
...
auto MpcTrajectoryPlanner::initialize(YAML::Node const& yaml,
...
auto MpcTrajectoryPlanner::plan(predictor::Snapshot const& snapshot, TimePoint center_time,
