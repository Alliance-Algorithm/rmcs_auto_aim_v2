...
auto make_setup_config() -> Solver::SetupConfig {
    ...
    auto Q = Eigen::Matrix2d { Eigen::Matrix2d::Zero() };
    Q(0, 0) = 9e6;
...
auto make_reference(double angle) -> Solver::StateTrajectory {
    auto reference = Solver::StateTrajectory { Solver::StateTrajectory::Zero() };
...
TEST(tiny_mpc_solver, solves_constant_reference) {
    ...
    const auto reference = make_reference(0.75);
    auto x0              = Solver::StateVector { reference.col(0) };
    const auto u_ref     = Solver::InputTrajectory::Zero();
    ...
    const auto state = solver.state<0, kHorizon / 2>();
    EXPECT_NEAR(state, 0.75, 1e-6);
}
...
TEST(tiny_mpc_solver, input_is_zero_for_constant_reference) {
    auto created = Solver::create(make_init_config());
    ASSERT_TRUE(created.has_value()) << created.error();
    auto solver = std::move(created).value();
    ...
    const auto input = solver.input<0, kHorizon / 2 - 1>();
    EXPECT_NEAR(input, 0.0, 1e-9);
}
