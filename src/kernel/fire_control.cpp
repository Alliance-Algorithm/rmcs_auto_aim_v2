...
struct FireControl::Impl {
    ...
    static constexpr auto kMinValidBulletSpeed = double { 10.0 };
    static constexpr auto kMaxIterateCount     = int { 5 };
    static constexpr auto kMaxFlyTimeThreshold = double { 0.001 };
    ...
    auto make_command(Armor3D const& armor) const -> std::optional<AimCommand> {
    ...
    auto solve_target(predictor::Snapshot const& snapshot) -> std::optional<ImpactTarget> {
        ...
        const auto impact_time = snapshot.time_stamp()
            + std::chrono::duration_cast<Duration>(
    ...
    auto solve(predictor::Snapshot const& snapshot, bool control, double current_yaw)
        ...
        if (!impact_target.has_value()) return std::nullopt;

        auto direct_command = make_command(impact_target->armor);
        if (!direct_command.has_value()) return std::nullopt;
