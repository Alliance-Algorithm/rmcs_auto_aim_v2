...
struct MpcPlanVisualizer::Impl {
    ...
    auto initialize(RclcppNode& visual_node) noexcept -> void {
        ...
        planned_pitch_pub =
            visual_node.details->make_pub<DebugVector3>(topic_prefix + "planned_pitch", qos::debug);
    }

    auto publish(const std::shared_ptr<rclcpp::Publisher<DebugVector3>>& publisher, double x,
        double y, double z) const noexcept -> bool {
        if (!node.has_value() || !publisher) return false;
...
auto MpcPlanVisualizer::initialize(RclcppNode& node) noexcept -> void { pimpl->initialize(node); }

auto MpcPlanVisualizer::publish_planned_yaw(
    double yaw, double yaw_rate, double yaw_acc) const noexcept -> bool {
    return pimpl->publish_planned_yaw(yaw, yaw_rate, yaw_acc);
}

auto MpcPlanVisualizer::publish_planned_pitch(
    double pitch, double pitch_rate, double pitch_acc) const noexcept -> bool {
